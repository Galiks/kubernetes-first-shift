...
(`job_builder.JobName` в `internal/api/job_builder.go`). Все параллельные запросы с одним ключом вычисляют одно
имя; победивший create проходит, проигравшие получают `409 AlreadyExists` →
`createJobIdempotent` читает Job по имени, сверяет `relayforge/request-hash`
...
поведение зафиксированы юнит-тестами (`internal/api/k8s_client_test.go`) и сценарием
...
В `createJobIdempotent` после `5xx/timeout` (статус 0 = клиентский таймаут)
...
`JobRegistry` (`internal/api/job_registry.go`, ручной ListWatch-цикл; per-Pod lock;
счёт = namespace-scoped watch + локальный
...
восстановление). Поэтому `/livez` проверяет только HTTP-процесс (Go `relayforge api`), а
