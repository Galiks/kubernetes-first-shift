...
- **Go 1.26** (модуль `relayforge`, `go.mod`: `go 1.26.0`);
...
- `helm/relayforge/` — действующий проект: `cmd/`, `internal/`, `go.mod`,
  `go.sum`, `Dockerfile`, `.dockerignore`, `chart/relayforge/`, `cluster/`,
...
Задание требует **зафиксировать версии зависимостей и базовых container
images** («Зафиксируй версии… локальные images через registry»). Поэтому в
`go.mod` версии жёстко закреплены, а базовые образы — точечные теги.

**Go-зависимости (`go.mod`):**

| Модуль | Версия | Назначение |
|---|---|---|
| `k8s.io/client-go` | `v0.32.0` | client к Kubernetes API |
| `k8s.io/api` | `v0.32.0` | типы объектов Kubernetes (`batch/v1` Job и др.) |
| `k8s.io/apimachinery` | `v0.32.0` | мета-типы, кодирование |
| `github.com/prometheus/client_golang` | `v1.24.1` | метрики `/metrics` |

Приложение — **один статический Go-бинарник** (модуль `relayforge`, `go 1.26`),
собирается из `./cmd/relayforge`. Внутри образа НЕТ Python, kubectl и shell
(см. §4, Dockerfile).

Container-образ (`Dockerfile`) — **multi-stage**:
- **builder**: `golang:1.26-alpine` (сборка `CGO_ENABLED=0` → статический бинарник
  `/relayforge`);
- **runtime**: `gcr.io/distroless/static-debian12:nonroot`, `USER 1000:1000`,
  бинарник копируется в `/relayforge` (`ENTRYPOINT` не задан — режим выбирает
  chart через exec-form `command`).
...
```dockerfile
# Runtime-стадия: без shell, без Python. ENTRYPOINT не задан — режим выбирает
# chart через exec-form command: ["/relayforge", "<mode>"].
COPY --from=builder /relayforge /relayforge
USER 1000:1000
CMD ["api"]
```

Маппинг режимов определён в `cmd/relayforge/main.go` (модуль `relayforge`):

| Режим | Пакет | Что делает |
|---|---|---|
| `api` | `internal/api` | принимает доставки (`POST /v1/deliveries`), показывает их (`GET /v1/deliveries/{id}`), служебные `/livez`,`/readyz`,`/metrics`, встроенная UI-панель `/ui` |
| `worker` | `internal/worker` | выполняет **одну** доставку внутри Kubernetes Job |
| `test-sink` | `internal/testsink` | изображает внешний HTTP-приёмник (обучаем режимам отказов) |
| `helm-test` | `internal/helmtest` | отправляет тестовую доставку и проверяет результат (hook Job `helm test`) |
| `cleanup` | `internal/cleanup` | pre-delete hook: чистит delivery-Jobs своего release при `helm uninstall` |

Запуск режима: `relayforge <режим>` (chart задаёт exec-form
`command: ["/relayforge", "api"]` в `templates/deployment-api.yaml`). Как и в
`deployment-api.yaml`, worker контейнер Job'а вызывается как
`command: ["/relayforge", "worker"]` (`internal/api/job_builder.go`).
...
В окружении должны быть: `docker`, `k3d`, `helm` (≥4.2), `go` (1.26), `kubectl`.
...
Путь в `internal/api/routes.go` (обработчик `POST /v1/deliveries`):
`auth → validate → canonicalize → создать Job`.

1. **auth** (`verifyToken`, `internal/api/auth.go`): читает client token из Secret,
   constant-time сравнение через `crypto/subtle.ConstantTimeCompare` (без
   timing-утечки), при неверном токене возвращает `401` **до** обращения к
   Kubernetes API. `/livez`, `/readyz` токен не требуют.
2. **validate** (`internal/api/validation.go`): `destination` из конфига release,
   `event_type` по `[a-z][a-z0-9_.-]{2,63}`, `payload` — JSON object, тело ≤ 16 KiB,
   неизвестное поле → `422`, клиент не передаёт URL приёмника.
3. **canonicalize** (`internal/canonical/canonical.go`): сериализация
   `destination`+`event_type`+`payload` в UTF-8 с отсортированными ключами и без
   незначащих пробелов; `1` и `1.0` — разные числа; NaN/Infinity/дубликаты/
   невалидный UTF-8 отклоняются. Байтово совместимо с прежним orjson-поведением
   (закреплено корпусом `testdata/canonical.jsonl`).
4. **create Job** (`createJobIdempotent`, `internal/api/k8s_client.go`): имя =
   `release + "-" + sha256(ключ)` (усечение до 63, `job_builder.JobName`), creation
...
`internal/worker/run.go` берёт параметры из env (`RELAYFORGE_DELIVERY_ID`,
...
- **Подпись** (`internal/worker/signer.go`): HMAC-SHA256 от
  `timestamp + "\n" + delivery_id + "\n" + body` (точные отправленные bytes).
  Значение: `{timestamp}\n{delivery_id}\n`. Проверяет `internal/testsink/verifier.go`
  (+ clock skew ≤ 5 мин).
- **Классификация ответа** (`internal/worker/classifier.go`, exit codes в
...
- **Retry/лимиты** (задаются в `job_builder.BuildJob`): `backoffLimit: 3`,
  `activeDeadlineSeconds: 300`, `ttlSecondsAfterFinished: 600`,
  `restartPolicy: Never`, `podFailurePolicy` на exit code 12.
- Логи — **JSON** в stdout (`internal/logging`, `slog` с deny-list), поле `extra`
  выкидывает payload/signature/secret/token/password. В логах нет payload/подписи.
...
`internal/testsink/routes.go`. Создаётся только при
...
- **Backpressure** (`internal/api/backpressure.go`, `internal/api/job_registry.go`):
  мягкий release-scoped лимит active Job (`api.backpressure.maxActiveJobs`) и макс.
  одновременных create на под (`maxConcurrentCreate`). Проверка+резервирование —
  атомарно под per-Pod семафором, счёт — namespace-scoped watch (ручной
  ListWatch-цикл `job_registry.go`) + локальный pending-учёт (без cluster-wide
  list). При превышении — `503 BACKPRESSURE` +
...
- **Cleanup (pre-delete hook)** (`templates/job-pre-delete-cleanup.yaml`,
  `internal/cleanup/run.go`): при `helm uninstall` масштабирует свой API
...
- **Metrics** (`internal/api/metrics.go`): Prometheus text format —
...
Юнит-тесты — `go test ./...` (пакеты `internal/api`, `internal/worker`,
`internal/testsink`, `internal/canonical`, `internal/body`). Покрывают:
канонизацию JSON (дифференциальный корпус `testdata/canonical.jsonl`),
детерминированное имя Job, HMAC framing и clock skew, client token/401 без K8s,
классификацию HTTP-статусов, преобразование Job status в ответ,
`409`/`403`/`429`/`5xx`/timeout, retry budget, конкурентный лимит, graceful
shutdown.
...
динамического Job, который создаёт API (`internal/api/job_builder.go`:
...
`internal/logging` (JSON-логгер, `slog` с deny-list) отбрасывает `payload`,
...
`imagePullPolicy: IfNotPresent` и digest (см. `internal/api/job_builder.go`, Dockerfile/§5d).
...
3. `cmd/relayforge/main.go` + `internal/api/routes.go` — точка входа и
   HTTP-контракт / flow создания доставки;
4. `internal/api/k8s_client.go` — клиент к K8s API, retry/409/именование;
5. `internal/api/job_builder.go` — как строится Delivery-Job;
6. `internal/worker/run.go` — выполнение одной доставки (exit-коды, HMAC);
7. `internal/worker/signer.go` — точный framing подписи;
8. `internal/testsink/routes.go` — режимы отказов и приёмник;
...
11. `internal/api/k8s_client_test.go` — граничные случаи K8s-клиента;
