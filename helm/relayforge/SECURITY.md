...
- Подпись (HMAC), signing key, client/control токены в логах и HTTP-ответах
  ...
  evidence/08).
- `logging.JsonFormatter` отбрасывает из extra-полей: `payload`, `signature`,
...
- Аутентификация API: client token из заранее созданного Secret, сравнение через
  `hmac.compare_digest`, 401 до обращения к Kubernetes API. `control-token`
...
(через Pod-level `fsGroup`, не privileged) и монтирует `emptyDir:/tmp` для Python
