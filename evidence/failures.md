...
### Команды диагностики: 
`kubectl get pods -A` или `kubetck -n pavel lab get pods`
...
### Симптом: 
`kubectl get service web -o wide` показывает web, но `kubectl get endpointslices` для web ENDPOINTS показывает <unset>
...
несовпадение label
### Исправление: 
выставить одинаковые label
### Как проверить восстановление: 
вызвать `kubectl get endpointslices`
